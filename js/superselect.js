...
;(function ($, window, document) {
    ...
    SelectList.prototype = {
        ...
        setSelectDisabled: function (element) {
            var that = this,
                $this = $(element),
                selectID = '#super-' + that.getSelectID($this);
            ...
            if (that.settings.enable) {
                $(selectID)
                    .click(function (event) {
                        ...
                        $(this).find('.select-down').toggleClass('select-up');
                ...
                selectItem.on('click', function (event) {
                    ...
                    $(that.element).value = $(this).data('value');
                    $(this.parentNode.parentNode.parentNode).find('.select-down').removeClass('select-up');
                    return false;
                }).hover(function () {
                    $(this).addClass('selected').siblings().removeClass('selected');
    ...
    $.fn[pluginName] = function (options) {
        ...
        var width = 177,
            height = 32;
        if ($(this) && $(this)[0] && $(this)[0].parentElement) {
            width = $($(this)[0].parentElement).width() < 177 ? 177 : $($(this)[0].parentElement).width();
            height = $($(this)[0].parentElement).height();
        }
    ...
    $(function () {
        $('.super-select').selectlist({
            enable: true,   //选择列表是否可用
            zIndex: 10,  //选择列表z-index值，如需兼容IE6/7,必须加此属性
            width: 150,   //选择列表宽度 默认为空 会自动获取父元素的宽度
            height: 34,  //选择列表高度 默认为空 会自动获取父元素的高度
            border: null,  //选择列表边框
            borderRadius: 16,  //选择列表圆角
            showMaxHeight: null,  //选择列表显示最大高度
            optionHeight: 34,   //选择列表单项高度
            triangleSize: 6,   //右侧小三角大小
            triangleColor: '#66ccff',  //右侧小三角颜色
            topPosition: false,  //选择列表项在列表框上部显示,默认在下边显示
            speed: 100   //选择列表框显示动画速度（毫秒）
        });
    });
